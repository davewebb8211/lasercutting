...
#
#
#  edited by Bernhard Kubicek to run on default emc-enabled ubuntu hardy without no further dependencies.
#  expanded by Bernhard Kubicek to create G-Code
#
#  ---------------------------------------------------------------------------
#
...
def involute2(r,  phi_start,phi_stop, steps):
    ox = cos(phi_start) * r;
    oy = sin(phi_start) * r;
    phi=phi_start
    x = r * (phi - phi_start)
...
#        line(cos(phi2)*(Dr/2), sin(phi2)*(Dr/2), cos(phi2)*(Dr/2-c), sin(phi2)*(Dr/2-c))
        line(cos(phi2)*(Dr/2-c), sin(phi2)*(Dr/2-c),cos(phi2)*(Dr/2), sin(phi2)*(Dr/2))
    ax, ay = involute(Dr/2, phi2, phi2 + phi_tr, 10)
    bx, by = involute2(Dr/2, phi3, phi3 - phi_tr, 10)
#    line(ax, ay, bx, by)
    if t > 0:
        axt, ayt = ax, ay
        bxt, byt = bx, by
...
if outfile != "":
    if outfile != "-":
        f = open(outfile+".ngc", "w")
    else:
        f = sys.stdout

    f.write("#1=400; feedrate\n")
    f.write("#2=5; up\n")
    f.write("#3=0; down\n")
    f.write("#4=1; tool number for using the cutter diameter compensation\n")
    f.write("#5=2; cutter diameter for initial move\n")
    f.write("\nG40 G21 G90 F#1\n\nG0 Z#2\n")
    f.write("\nG0 X[%f+#5] Y[%f+#5] ; initial cutter compensation tail\n"%(linedata[0],linedata[1]))
    f.write("G42 D#4\n")
    f.write("\nG0 X%f Y%f \n"%(linedata[0],linedata[1]))
    f.write("\nG1 Z #3\n")
    oldx=0
